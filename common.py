...
def get_jobs_db():
    return get_db(constants.EVAL_JOBS_COLLECTION_NAME)


def get_instances_db():
    return get_db(constants.EVAL_INSTANCES_COLLECTION_NAME)
...
def add_botleague_host_watch():
    ...
    BOTLEAGUE_LIAISON_HOST = db.get('BOTLEAGUE_LIAISON_HOST')

    log.info(f'Initialized botleague liaison host to {BOTLEAGUE_LIAISON_HOST}')
    ...
    def on_botleague_host_change(col_snapshot, changes, read_time):
        ...
        for change in changes:
            ...
            if db_host_value and db_host_value.BOTLEAGUE_LIAISON_HOST:
                BOTLEAGUE_LIAISON_HOST = db_host_value.BOTLEAGUE_LIAISON_HOST
            else:
                # Fallback if it's not set in DB
                BOTLEAGUE_LIAISON_HOST = constants.BOTLEAGUE_LIAISON_HOST
            log.success(f'Changed botleague host to {BOTLEAGUE_LIAISON_HOST}')
